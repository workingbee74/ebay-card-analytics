...
def ensure_ebay_oauth_table():
    ...
    with psycopg.connect(DATABASE_URL) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_auction_watch_current_end
                ON auction_watch_current (item_end_date)
            """)
    
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_auction_watch_current_priority
                ON auction_watch_current (
                    urgency_score DESC,
                    momentum_score DESC,
                    demand_score DESC
                )
...
def calculate_disposition(
    ...
    if (
        ...
        and (
            ...
            or (
                serial_numbered_to
                and serial_numbered_to <= 25
            )
        )
    ):
        action = "HOLD"
        reasons.append("Premium Bowman/prospect profile favors holding for further upside")
    
    elif (
        gain_loss_pct is not None
        and gain_loss_pct >= 25
        ...
        if (
            gain_loss_pct is not None
            and gain_loss_pct >= 15
        ):
            action = "SELL - BIN + BEST OFFER"
            reasons.append("Strong liquidity and positive gain favor a controlled BIN exit")
        else:
            action = "SELL - AUCTION"
            reasons.append("Strong liquidity supports competitive auction bidding")
    
    elif liquidity == "MODERATE":
        action = "SELL - BIN + BEST OFFER"
        reasons.append("Moderate liquidity favors a patient BIN strategy")
...
def analyze_one_buying_opportunity():
    ...
    if (
        ...
        and psa9_weighted_ratio is not None
        and psa9_related_confidence in ("MEDIUM", "HIGH")
    ):
        inferred_psa9_value = round(
    ...
    "exact_grade_prices": {
        ...
        "related_evidence": {
            ...
            "inferred_psa9_value": inferred_psa9_value,
...
def pipeline_top100_test():
    ...
    for row in rows:
        ...
        for player in players:
            player_entity = player.get("playerEntity", {})
            player_ref = player_entity.get("player", {}).get("__ref", "")
